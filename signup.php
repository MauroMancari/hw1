<?php
    ...
    #VERIFICO L'ESISTENXZA DEI DATI POST
        ...
        !empty($_POST["surname"]) && !empty($_POST["confirm_password"]) && !empty($_POST["allow"]))
    {
        ...
        #USERNAME -> CONTROLLA CHE L'USERNAME RISPETTI IL PATTER SPECIFICATO
        if(!preg_match('/^[a-zA-Z0-9_]{1,15}$/', $_POST['username'])) {
            $error[] = "Username non valido";
        } else {
            $username = mysqli_real_escape_string($conn, $_POST['username']);
            #CERCO SE L'USERNAME E' GIA' ESISTENTE
            $query = "SELECT username FROM 
                ( SELECT username FROM users UNION ALL SELECT 'search' UNION ALL SELECT 'create' UNION ALL SELECT 'home' ) 
        ...
        #FACCIO LA REGISTRAZIONE NEL DATABASE.
        if (count($error) == 0) {
            ...
            if (mysqli_query($conn, $query)) {
                $_SESSION["_applenet_username"] = $_POST["username"];
                $_SESSION["_applenet_user_id"] = mysqli_insert_id($conn);
                mysqli_close($conn);
                header("Location: home.php");
                exit;
            } else {
...
<html>
    <head>
        ...
        <title>Applenet - Iscriviti</title>
    </head>
    <body>
        <main>
        ...
        <section class="main_right">
            ...
            <form name='signup' method='post' enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="fileupload">
                    ...
                    <span>Le dimensioni del file superano 7MB</span>
                </div>
                <div class="allow"> 
                    <div><input type='checkbox' name='allow' value="1" <?php if(isset($_POST["allow"])){echo $_POST["allow"] ? "checked" : "";} ?>></div>
                    <div><label for='allow'>Steve Jobs vi osserva, vi ruberà tutti i dati</label></div>
                </div>
                <div class="submit">
